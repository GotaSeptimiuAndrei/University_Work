...
BEGIN
    BEGIN TRAN;
    BEGIN TRY
        EXEC addRowSong 'Domino', 'pop', '2011-12-26', 55, 1487130, 540004;
        EXEC addRowPlaylist 'Pop hits';
        EXEC addRowSongInPlaylist 'Domino', 'Pop hits'; 
    END TRY
    BEGIN CATCH
        ...
        ROLLBACK TRAN;
        EXEC sp_log_changes '', '', 'rolledback all data', @errorMsg;
        RETURN;
    END CATCH
    COMMIT TRAN;
END
...
BEGIN
    ...
    BEGIN TRY
        EXEC addRowSong 'Domino', 'pop', '2011-12-26', 55, 1487130, 540004;
        EXEC addRowPlaylist 'Pop hits';
        EXEC addRowSongInPlaylist 'Do', 'Pop hits'; -- This will fail due to missing song 'Do'
    END TRY
    BEGIN CATCH
        DECLARE @errorMsg NVARCHAR(MAX);
        SELECT @errorMsg = ERROR_MESSAGE();
