...
BEGIN
	...
	BEGIN CATCH
		...
		ROLLBACK TRAN
		EXEC sp_log_changes '', '', 'rolledback song data', @errorMessage
	END CATCH
END
...
BEGIN
	...
	BEGIN CATCH
        ...
        SELECT @errorMessage = ERROR_MESSAGE()
		ROLLBACK TRAN
		EXEC sp_log_changes '', '', 'rolledback playlist data', @errorMessage
	END CATCH
END
...
BEGIN
	...
	BEGIN CATCH
        ...
        SELECT @errorMessage = ERROR_MESSAGE()
        ROLLBACK TRAN;
        EXEC sp_log_changes '', '', 'rolledback song in playlist data', @errorMessage;
	END CATCH
END
